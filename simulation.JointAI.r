...
A<-NULL
for (i in 1:n.sim){ 
imp<-lmer_imp(dbp ~ age + sex + bmi, data=M[((i-1)*nrow(M[which(M$dataset==i),])+1):((i-1)*nrow(M[which(M$dataset==i),])+nrow(M[which(M$dataset==i),])),]
, random = ~ 1 | id, n.chains = 3, n.adapt = 100, n.iter = 1000, monitor_params = c(imps = TRUE),thin = 1, seed = 1234)
B<-get_MIdat(imp, m = 10, include = FALSE, minspace = 50,seed = 1234)
A<-rbind(B,A)
}
write.table(A,"simulation.JointAI.csv",sep=",",col.names=T,row.names=F)
A<-read.csv("simulation.JointAI.csv",header=TRUE,na.string="NA")
names(A)
str(A)
