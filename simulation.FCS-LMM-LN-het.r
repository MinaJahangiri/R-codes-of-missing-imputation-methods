...
predmat["id",] <- c(0,0,0,0,0,0,0)
predmat["time",] <- c(-2,0,0,0,0,0,0)
predmat["age",] <- c(-2,0,0,0,0,0,0)
predmat["dbp", ] <- c(-2,0,1,1,1,0,0)
predmat["bmi",] <- c(-2,0,1,1,0,1,0)
predmat["sex",] <- c(-2,0,0,0,0,0,0)
predmat["dataset",] <- c(0,0,0,0,0,0,0)

impmeth <- rep("",V)
names(impmeth) <- vars
impmeth["dbp"] <- "2l.continuous"
impmeth["bmi"] <- "2l.continuous"

data1[[i]]<-complete(mice(data=M[((i-1)*nrow(M[which(M$dataset==i),])+1):((i-1)*nrow(M[which(M$dataset==i),])+nrow(M[which(M$dataset==i),])),]
,method=impmeth,predictorMatrix=predmat,diagnostics=TRUE, maxit=5, m=10,remove.collinear=FALSE), action = "long", include = FALSE)
}

write.table(as.data.frame(do.call(rbind, data1)),"simulation.miceadds.continuous.csv",sep=",",col.names=T,row.names=F)
A<-read.csv("simulation.miceadds.continuous.csv",header=TRUE,na.string="NA")

names(A)
str(A)
A<-within(A, sex<-factor(sex))  # sex is a factor
names(A)[names(A) == ".imp"] <- "imp"
names(A)
head(A)
tail(A)
nrow(A)
