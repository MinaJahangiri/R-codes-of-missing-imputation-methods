...
A<-write.mitmlSAV(mitmlComplete(panImpute(data=M[((i-1)*nrow(M[which(M$dataset==i),])+1):((i-1)*nrow(M[which(M$dataset==i),])+nrow(M[which(M$dataset==i),])),]
, formula=formula,n.burn=1000,n.iter=1000, m=10,seed=1234), print="all"), filename="mitml.pan.DBP.sav")
A<-write.table(read.spss("mitml.pan.DBP.sav"), file="mitml.pan.DBP.csv", quote = TRUE, col.names=T,row.names=F,sep = ",")
data1[[i]]<-read.csv("mitml.pan.DBP.csv",header=TRUE, na.string ="NA")
}
write.table(as.data.frame(do.call(rbind,data1)), file="simulation.mitml.pan.csv", quote = TRUE, col.names=T,row.names=F,sep = ",")
A<-read.csv("simulation.mitml.pan.csv",header=TRUE)
names(A)
nrow(A)
summary(A)
str(A)
