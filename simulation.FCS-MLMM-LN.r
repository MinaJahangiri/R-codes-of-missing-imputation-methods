...
predM1
ind.clust<-1   
method<-find.defaultMethod(M,ind.clust);method
res.mice.par<-mice.par(M[((i-1)*nrow(M[which(M$dataset==i),])+1):((i-1)*nrow(M[which(M$dataset==i),])+nrow(M[which(M$dataset==i),])),]
, m = 10, predictorMatrix = predM1,method = method, maxit=5,seed=1234,nnodes=3)
B<-complete(res.mice.par,action = "long", include = FALSE)
A<-rbind(B,A)
}
A<-read.csv("simulation.micemd.csv",header=TRUE,na.string="NA")
names(A)
str(A)
