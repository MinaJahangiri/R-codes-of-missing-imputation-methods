...
#bmi
copy.mean2.bmi<-NULL
for (i in 1:n.sim){ #run a loop over the dataframes in the list
...
copy.mean1.bmi<-as.data.frame(unname(imp.bmi))
copy.mean2.bmi<-rbind(copy.mean2.bmi,copy.mean1.bmi)
}
names(copy.mean2.bmi)
colnames(copy.mean2.bmi)<-c("bmi0","bmi1","bmi2","bmi3","bmi4","bmi5")
...
copy.mean1.dbp<-as.data.frame(unname(imp.dbp))
copy.mean2.dbp<-rbind(copy.mean2.dbp,copy.mean1.dbp)
}
names(copy.mean2.dbp)
colnames(copy.mean2.dbp)<-c("dbp0","dbp1","dbp2","dbp3","dbp4","dbp5")
