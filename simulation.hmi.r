...
B<-complete(hmi(data=M[((i-1)*nrow(M[which(M$dataset==i),])+1):((i-1)*nrow(M[which(M$dataset==i),])+nrow(M[which(M$dataset==i),])),]
,model_formula=model,m = 10, nitt =1100,burnin = 1000), "long", include = FALSE)
A<-rbind(B,A)
}
write.table(A,"simulation.hmi.csv",sep=",",col.names=T,row.names=F)
A<-read.csv("simulation.hmi.csv",header=TRUE,na.string="NA")
nrow(A)
names(A)
