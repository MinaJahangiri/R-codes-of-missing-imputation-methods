...
# plots
#######################################
pdf("miceadds.continuous.impplot.DBP.pdf")
plot(imp)
dev.off()

pdf("miceadds.continuous.bwplot.DBP.pdf")
bwplot(imp)
dev.off()

pdf("miceadds.continuous.stripplot.DBP.pdf")
stripplot(imp)
dev.off()

pdf("miceadds.continuous.densityplot.DBP.pdf")
densityplot(imp)
dev.off()

pdf("miceadds.continuous.densityplotdbp.DBP.pdf")
densityplot(imp, ~dbp|.imp)   #blue is observed, red is imputed
dev.off()

pdf("miceadds.continuous.densityplotbmi.DBP.pdf")
...
write.table(data,"miceadds.continuous.DBP.csv",sep=",",col.names=T,row.names=F)
B<-read.csv("miceadds.continuous.DBP.csv",header=TRUE,na.string="")
names(B)
