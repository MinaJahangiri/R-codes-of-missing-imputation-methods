...
############################################
# Multiple imputation using jomo.smc.common
############################################
formula<-as.formula(dbp~age+sex+bmi+(1|id))

imp<-jomo.smc(formula,data=A, nburn=5000, nbetween=1000, nimp=20, meth="common",family="gaussian",model="lmer")

# convergence of the first element of beta
imp1<-jomo.smc.MCMCchain(formula, data=A,nburn=5000, meth="common", family="gaussian",model="lmer")

pdf("jomo.smc.common.DBP.pdf")
par(mfrow=c(2,2))
...
# write data
###########################################
write.table(imp,"jomo.smc.common.DBP.csv",sep=",",col.names=T,row.names=F)
B<-read.csv("jomo.smc.common.DBP.csv",header=TRUE, na.strings ="NA")
names(B)
B<-B[,-c(4,8)]
names(B)
str(B)
