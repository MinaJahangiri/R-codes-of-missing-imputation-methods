...
, nburn=1000, nbetween=1000, nimp=10, meth="common",family="gaussian",model="lmer")
A<-rbind(B,A)
}
write.table(A,"simulation.jomo.smc.common.csv",sep=",",col.names=T,row.names=F)
A<-read.csv("simulation.jomo.smc.common.csv",header=TRUE, na.strings ="NA")
names(A)
A<-A[,-8]
