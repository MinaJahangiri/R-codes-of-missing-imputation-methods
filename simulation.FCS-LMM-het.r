...
predM1 <- predM
predM1["dbp",] <- c(-2,0,1,1,1,0,0)
predM1["time",] <- c(-2,0,0,0,0,0,0)
predM1["dataset",] <- c(0,0,0,0,0,0,0)
predM1["age",] <- c(-2,0,0,0,0,0,0)
predM1["bmi",] <- c(-2,0,1,1,0,1,0)
predM1["sex",] <- c(-2,0,0,0,0,0,0)
predM1["id",] <- c(0,0,0,0,0,0,0)
...
impM1["bmi"] <- "2l.norm"
impM1
data1[[i]]<-complete(mice(M[((i-1)*nrow(M[which(M$dataset==i),])+1):((i-1)*nrow(M[which(M$dataset==i),])+nrow(M[which(M$dataset==i),])),]
, m = 10, predictorMatrix = predM1,Method = impM1, maxit=5,diagnostics=TRUE,seed=1234)
, action = "long", include = FALSE)
}
write.table(as.data.frame(do.call(rbind, data1)),"simulation.mice.het.csv",sep=",",col.names=T,row.names=F)
A<-read.csv("simulation.mice.het.csv",header=TRUE,na.string="NA")
names(A)
str(A)
